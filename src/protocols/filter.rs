...
impl<S: Store + Send + Sync> FilterProtocol<S> {
    ...
    fn insert_hashes(&mut self, peer: PeerIndex, _hashes: packed::GcsFilterHashes) {
        ...
        if stop_num >= parent_num && (stop_num - parent_num) as usize == hash_len {
            self.filter_hashes
                .write()
                .insert(peer.clone(), FilterHashes::new(_hashes, true));
            //send msg
            self.inner_sender
                .send(GcsMessage::GetFilters(peer.clone()))
                .unwrap();
        }
    }
    ...
    fn insert_checkpoints(&mut self, peer: PeerIndex, _check_points: packed::GcsFilterCheckPoint) {
        ...
        if let Some(last_filter_hash) = _check_points
            .filter_hashes()
            .get(_check_points.filter_hashes().len() - 1)
        {
            let mut _times = 1;
            ...
            if _times == valid_peers || _times > 1 && _times > (data_len + 1) / 2 {
                ...
                self.check_points
                    .write()
                    .insert(peer.clone(), CheckPoints::new(_check_points, true));
                self.inner_sender
                    .send(GcsMessage::GetFilterHashes(peer.clone()))
                    .unwrap();
            } else {
                self.check_points
                    .write()
                    .insert(peer.clone(), CheckPoints::new(_check_points, false));
            }
        }
    }
}
                ...
                if let Ok(msg) = self.inner_receiver.try_recv() {
                    match msg {
                        GcsMessage::GetCheckPoint => {
                            ...
                            info!(
                                "notify get checkpoint: stop_num is {}, interval is {}",
                                stop_num, interval
                            );
                            ...
                            peers.into_iter().for_each(|peer| {
                                if let Err(err) = nc.send_message_to(peer, message.as_bytes()) {
                                    debug!(
                                        "GcsFilterProtocol send GetGcsFilterCheckPoint error: {:?}",
                                        err
                                    );
                                }
                            });
                        ...
                        GcsMessage::GetFilterHashes(peer) => {
                            ...
                            let mut start_block_num = self
                                .store
                                .get_header(start_block_hash)
                            ...
                            let stop_number = self
                                .store
                                .get_header(stop_hash.clone())
                                .expect("stored should be OK")
                                .expect("get header stored")
                                .number();
                            ...
                            let peers: Vec<PeerIndex> = self
                                ...
                                .collect();
                            info!(
                                "notify get filter hashes:start_num is {},stop_hash is {}",
                                start_block_num, stop_hash
                            );
                            peers.into_iter().for_each(|peer| {
                                if let Err(err) = nc.send_message_to(peer, message.as_bytes()) {
                                    debug!(
                                        "GcsFilterProtocol send GetGcsFilterhash error: {:?}",
                                        err
                                    );
                                }
                            });
                        ...
                        GcsMessage::GetFilters(peer) => {
                            ...
                            let stop_hash =
                                self.filter_hashes.read().get(&peer).unwrap().stop_hash();
                            start_block_num = start_block_num + 1;
                            let stop_block_num = self
                                .store
                                .get_header(stop_hash.clone())
                                .expect("stored should be OK")
                                .expect("store head")
                            ...
                            */
                            ...
                            loop {
                                ...
                                tmp_stop_hash = self
                                    .store
                                    .get_block_hash(tmp_stop_num.clone())
                                    .expect("stored should be OK")
                                    .expect("store hash");
                                ...
                                if let Err(err) =
                                    nc.send_message_to(peer.clone(), message.as_bytes())
                                {
                                    debug!(
                                        "GcsFilterProtocol send peer {} GetGcsFilters error: {:?}",
                                        err, peer
                                    );
                                }
            ...
            SEND_GET_GCS_CHECKPOINT_TOKEN => unsafe {
                match Time_Get_CheckPoint {
                    Some(now) => {
                        ...
                        if now.duration_since(Time_Get_CheckPoint.unwrap()) > five_secs {
        ...
        match message.as_reader() {
            packed::GcsFilterMessageUnionReader::GcsFilter(reader) => {
                ...
                */
                ...
                let compare_result = match self.filter_hashes.read().get(&peer) {
                    Some(value) => {
                        stop_num = self
                            .store
                            .get_header(value.stop_hash())
                            .expect("store should be OK")
                            .expect("store header")
                            .number();
                        ...
                        if filter_hash
                            == value
                                .filter_hashes()
                                .get(value.filter_hashes().len() - gap_len - 1)
                                .unwrap()
                        {
                            true
                        } else {
                ...
                if compare_result == true {
                    self.store
                        .insert_gcsfilter(gcs_filter)
                        .expect("store gcs filter should be OK");
                    self.store
                        .insert_record(block_num.clone(), block_hash)
